...
### 실제 수익률 (향후 전환)
매일 08:30·16:00 브리핑 때마다 `ASSET_SNAPSHOT:{YYYYMMDD}` 키로 일별 총자산/미실현/누적실현 스냅샷을 저장합니다. 이 데이터가 충분히 쌓이면 회계 항등식으로 입출금을 자동 역산해 Modified Dietz 기반 **실제 수익률**로 전환됩니다.

```
CashFlow[D] = total[D] − total[D-1] − ( Δrealized_cum[D] + ΔU[D] )
```
